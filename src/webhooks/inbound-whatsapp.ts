 ...
 * Flow: parse body → strip prefix → find agent → forward to callback → queue dead letter on failure → return TwiML.
 */
  ...
  fetch(url, {
    ...
    .then((resp) => {
      if (resp.ok) {
        ...
        logger.warn("inbound_whatsapp_callback_error", { url, status: resp.status });
        queueDeadLetter(deadLetterCtx, `Callback returned ${resp.status}`);
      }
    })
    .catch((err) => {
      const errMsg = err instanceof Error ? err.message : String(err);
      logger.warn("inbound_whatsapp_callback_failed", { url, error: errMsg });
